...
\newpage
	...
	\begin{itemize}
		\item[] $TL =$ \textbf{une file de thread telle que :} $\forall$ $tl \in TL~|~tl = T$ avec :
		\begin{itemize}
			\item[] $T = \langle I,S,E,C,D,L\rangle$ \textbf{le thread courant avec :}
			\begin{itemize}
				\item[] $b,s,n$ = une constante ou un identifiant de signal (un entier)
				\item[] $V = b$	
				\begin{itemize}
					\item[|] $\langle\langle X,C' \rangle E\rangle$
					\item[|] $[c,param_{1},...,param_{n}]$ avec n le nombre de paramètre pour le constructeur $c$
				\end{itemize}
				\item[] $I =$ un entier représentant l'identifiant du thread
				\item[] $S =  \emptyset$ 
				\begin{itemize}
					\item[|] $V~S$
					\item[|] $[c,var_{1}/pattern_{1},...,var_{n}/pattern_{n}]~S$ avec n le nombre de paramètre pour le constructeur $c$
				\end{itemize}
				\item[] $E = \{...,\langle X,V\rangle,...\}$
				\item[] $C = \epsilon$
				\begin{itemize}
				...
				\begin{itemize}
					\item[|] $\langle S,E,C,D\rangle$~~~~~~~~~~~~~~~~(une sauvegarde liée à une abstraction)
				\end{itemize}
				\item[] $L = \{...,\langle constructeur,nombre de parametre\rangle,...\}$
			\end{itemize}	
		\end{itemize}
	...
	\begin{itemize}
		\item[] Dans tous les cas :
		\item[] \begin{center}
			$\dfrac{\langle S,E,C,D,L\rangle \longrightarrow_{TTSI} \langle S',E',C',D',L'\rangle}{\langle\langle I,S,E,C,D,L\rangle,TL,SI,IP\rangle \longrightarrow_{TTSI} \langle\langle I,S',E',C',D',L'\rangle,TL,SI,IP\rangle}$
		\end{center}
		\item[]
		\item[] Si la règle utilisée n'est pas \textbf{Construction de type} :
		\item[] \begin{center}
			$\dfrac{\langle S,E,C,D\rangle \longrightarrow_{TTSI} \langle S',E',C',D'\rangle}{\langle\langle I,S,E,C,D\rangle,TL,SI,IP\rangle \longrightarrow_{TTSI} \langle\langle I,S',E',C',D'\rangle,TL,SI,IP\rangle}$
		\end{center}
		\item[]
	...
	\begin{enumerate}
		...
		\end{itemize}
		\item[] \textbf{Partie pour les types :}
		\item[]
		\begin{itemize}
			\item[] \textbf{Construction de type :}
			\smallbreak
			$\langle b~V^{n}~...~V^{1}~S,E,build~C,D,\{...,\langle b,n\rangle,...\}\rangle
			\longrightarrow_{TTSI} 
			\langle [b,V^{1},...,V^{n}]~S,E,C,D,\{...,\langle b,n\rangle,...\}\rangle$
			\item[]
			\item[] \textbf{Comparer deux types :}
			\smallbreak
			$\langle [c',...]~[c,...]~S,E,compare~C,D,L\rangle
			\longrightarrow_{TTSI} 
			\langle \langle bool,E\rangle~S,E,C,D,L\rangle$ 
			\\si $c'=c$ alors $bool = \langle x,\langle y,x\rangle\rangle$ sinon $bool = \langle x,\langle y,y\rangle\rangle$ 
			\\ avec $[c',...]$ un type et $[c,...]$ un pattern
			\item[]
			\item[] \textbf{Décomposer un type via pattern :}
			\smallbreak
			$\langle [c',...]~[c,...]~S,E,destruct~C,D\rangle
			\\\longrightarrow_{TTSI} 
			 \langle res_{n}~var/pattern_{n},E,destruct,\langle ...\langle res_{1}~var/pattern_{1},E,destruct,\langle S,E,C,D\rangle\rangle...\rangle\rangle$ 
			\\ avec $[c',...]$ un type et $[c,...]$ un pattern
			\item[]
			\item[] \textbf{Décomposer un type via variable :}
			\smallbreak
			$\langle res~var~S,E,destruct~C,D\rangle
			\longrightarrow_{TTSI}$ 
			\\ si $C = \epsilon$ alors $\langle S,E~\langle var,res\rangle,destruct,D\rangle$ sinon $\langle S,E~\langle var,res\rangle,C,D\rangle$
			\item[]
			\item[] \textbf{Décomposition finie :}
			\smallbreak
			$\langle \epsilon,E,destruct,\langle S,E',C,D \rangle\rangle
			\longrightarrow_{TTSI} \langle S,E'\cup E,C,D\rangle$ 
			\item[]
		\end{itemize}
		\item[] \textbf{Partie commune :} Quand on ajoute des règles dans une machine déjà existante, le plus délicat est de ne pas avoir de conflits dans les règles. Pour cela, on définit des règles exprès pour faire la liaison entre ce qui existait et ce que l'on ajoute. 
		\item[]
