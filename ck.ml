...
module CKMachine =
  struct
    ...
    let string_of_ck expr registre = "("^(string_of_expr expr)^" , "^(string_of_registre registre)^")\n" 


    (* Affiche un état de la machine CK *)
    let afficherCK expression registre = printf "MachineCK : %s" (string_of_ck expression registre)
    ...
    let rec machineCK expression registre = 
                                  ...
                                  if (estVide liste_expr1)
                                    then 
                                      begin
                                        ...
                                        if (for_all estConst newliste) then machineCK (calcul op (  rev (convert_liste_expr_liste_int newliste))) mt else raise EtatInconnu
                                      end
                                    else 
                                      begin
                                        let newliste =  append [expr] liste_expr in machineCK (getPremElem liste_expr1) (Opd((newliste,op),(enleverTete liste_expr1),mt))
                                      end
      ...
      match (expression,registre) with
        ...
        | (Const b,registre)        ->   if (estMT registre) then Const b else testRegistre registre (Const b)
    
        | (Abs(abs,expr),registre)  ->   if (estMT registre) then Abs(abs,expr) else testRegistre registre (Abs(abs,expr))
        
        | ((Var var),registre)      ->   testRegistre registre (Var var)

    
    (* Lance et affiche le résultat de l'expression *)
    let lancerCK expression = printf "Le résultat est %s \n" (string_of_expr (machineCK expression MT))
