  ...
  struct
    ...
    let rec beta_red expression = 
      match expression with 
        ...
        | App(Abs(abs,expr1),expr2)  ->   if (estVariable expr2) then (reduction abs expr1 expr2) else  (App(Abs(abs,(beta_red expr1)),(beta_red expr2)))
      ...
      let rec  delta_red expression =
        match expression with 
          ...
          | Op(op, liste_expr)  ->   if (for_all estConst liste_expr) 
                                      then 
      ...
      let alpha_eq liste_pair_lie expression1 expression2 =
        let rec aux expr1 expr2 =
          match (expr1,expr2) with
            ...
            | (Var var1 , Var var2)                      ->   if ((estDansUnCouple var1 liste_pair_lie) || (estDansUnCouple var2 liste_pair_lie)) then (mem (var1,var2) liste_pair_lie) else true
