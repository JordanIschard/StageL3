  ...
  struct
    ...
    let rec beta_red expression = 
      ...
      let rec n_red expression =
        ...
        let newExpr = (delta_red (beta_red expression)) in
        if (equalExpr expression newExpr || estErreur newExpr) then Printf.printf "->  %s" (string_of_expr newExpr) else n_red newExpr
