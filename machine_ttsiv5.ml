  ...
  struct
    ...
    let rec convert_to_machine_language expression = 
      ...
      match expression with
        ...
        | Lang_ttsi.ISWIM.App (expr1,expr2)                     ->  append (convert_to_machine_language expr1) 
        ...
        | Lang_ttsi.ISWIM.Present (variable,expr1,expr2)        ->  [Variable variable;Abstraction("",convert_to_machine_language expr1)
    ...
    let transition machine =
      match machine with
        ...
        | Machine(Thread(i,s,e,[],Empty),[],si,ip)                        
        ...
        | Machine(Thread(i,Const n::Const b::s,e,Put::c,d),tl,si,ip)           ->   let (st,new_si) = put_value si i n b in
                                                                                      Machine(Thread(i,s,e,c,d),append tl st,new_si,ip)
        ...
        (*** Partie types ***)
          ...
          (* Création d'un type *)
        | Machine(Thread(i,Const id::Const nbr::s,e,Build::c,d),tl,si,ip)      ->   let (res,new_s) = create s nbr in 
                                                                                      Machine(Thread(i,Type(id,res)::new_s,e,c,d),tl,si,ip)

          (* Comparer deux types *)
        | Machine(Thread(i,t2::t1::s,e,Compare::c,d),tl,si,ip)                 ->   let res = compare t1 t2 e in 
    ...
    let rec machine m afficher =
      match m with
          Machine(Thread(id,resultat,env,[],Empty),[],[],ip)  ->   printf "Le résultat est %s \n" (string_of_stack resultat)
