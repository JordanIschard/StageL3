...
module SECDMachine =
  struct
    ...
    type c =
      ...
      | Throw_SECD of int
      | Catch_SECD of int * c list * (variable * c list) 
    ...
    let rec secdLanguage_of_exprISWIM expression =
      match expression with
        ...
        | Catch(erreur,expr1,(abs,expr2))  ->   [Catch_SECD(erreur,(secdLanguage_of_exprISWIM expr1),(abs,(secdLanguage_of_exprISWIM expr2)))]
    ...
    let rec string_of_control_string expression =
      match expression with
        ...
        | Catch_SECD(erreur,expr1,(abs,expr2))::t  ->    "try "^(string_of_control_string expr1)^" catch "^(message_of_erreur erreur)
    ...
    let rec machineSECD machine =
      ...
      let rec testHandler handler erreur =
        match handler with
      ...
      match machine with
        ...
        | MachineSECD(s,e,(Prim(op))::c,w,st,si,d,h)                                     ->
                                        begin
                                          ...
                                          try   machineSECD (MachineSECD (
                                                ...
                                                , e , c , w , st , si , d , h ))
                                          with  FormatOpErreur -> machineSECD (MachineSECD([],[],[Throw_SECD(5)],[],[],[],Vide_D,h))
                                        end

        | MachineSECD(s,e,(Catch_SECD(erreur,c1,(abs,c2)))::c,w,st,si,d,h)               -> 
                                        machineSECD (MachineSECD( s , e ,(append c1 c) , w , st , si , d , SaveHandler(erreur,(s,e,(append [Pair(abs,c2)] c),w,st,si,d,h))))
        
        (* Si on a une abstraction dans la chaîne de contrôle, on la mets dans la pile *)
        | MachineSECD(s,e,(Pair(abs,control_string))::c,w,st,si,d,h)                     -> 
                                        machineSECD (MachineSECD( (Fermeture_secd([Pair(abs,control_string)],e) :: s) , e , c , w , st , si , d , h ))
        ...
        | MachineSECD(v::( Fermeture_secd([Pair(abs,c1)],e1))::s,e,(Ap)::c,w,st,si,d,h)  -> 
                                        begin
                                          ...
                                          with  EtatEnvInvalide -> machineSECD (MachineSECD([],[],[Throw_SECD 4],[],[],[],Vide_D,h))
                                        end
        ...
        | MachineSECD(s,e,(Bspawn)::c,w,st,si,d,h)                                       -> 
                                        begin
                                          ...
                                          with  FinSpawnNonTrouvable -> machineSECD (MachineSECD([],[],[Throw_SECD(6)],[],[],[],Vide_D,h))
                                        end
        ...
        | MachineSECD(s,e,(Signal_SECD(signal,c1))::c,w,st,si,d,h)                       -> 
                                        begin
                                          ...
                                          with  SignalDejaInit -> machineSECD (MachineSECD([],[],[Throw_SECD (8)],[],[],[],Vide_D,h))
                                        end
                                            ...
                                            if (estEmit si signal)
                                              ...
                                              (* le signal attendu n'est pas émit, plusieurs choix s'offrent à nous *)
                                                ...
                                                match w with
                                                  ...
                                                  (* elle est vide, du coup on mets à vide tout excepté la liste de bloqué où l'on ajoute un élément 
                                                     qui est une sauvegarde de l'état de la machine avec le signal attendu *)
                                                    []                   -> 
                                                                  machineSECD (MachineSECD( [] , [] , [] , [] , (append st [(signal,(Save(s,e,(Present_SECD(signal,c1,c2))::c,d)))]) , si , Vide_D , h))

                                                  (* elle n'est pas vide, du coup on sauvegarde l'état avec le signal attendu dans la liste des bloqués 
                                                     et on remplace par la sauvegarde en tête de liste de la liste en attente *)
                                                  | Save(s,e,c3,d)::t    -> 
                                                                  machineSECD (MachineSECD( s , e , c3 , t , (append st [(signal,(Save(s,e,(Present_SECD(signal,c1,c2))::c,d)))]) , si , Vide_D , h))

                                                  (* elle n'est pas de la forme attendu du coup on lève une exception *)
                                                  | _                    ->   machineSECD (MachineSECD([],[],[Throw_SECD(3)],[],[],[],Vide_D,h))
                                          
                                          (* le signal n'est pas initialisé donc on lève une exception *)
                                          else machineSECD (MachineSECD([],[],[Throw_SECD (7)],[],[],[],Vide_D,h))
        ...
        | MachineSECD(s,e,(Emit_SECD(signal))::c,w,st,si,d,h)                            -> 
                                        machineSECD (MachineSECD( Remp::s , e , c , (append (reveil signal st) w) , (resteStuck signal st) , (ajoutSI si signal) , d , h ))
        ...
        | MachineSECD([],[],[],[],st,si,Vide_D,h)                                        -> 
                                        begin 
                                          ...
                                          with  EtatStuckInvalide -> machineSECD (MachineSECD([],[],[Throw_SECD 2],[],[],[],Vide_D,h))
                                        end
