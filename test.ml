...
let renommer var terme1 terme2 =
  let varlibre = (retirerEl var (List.append (libre terme1) (libre terme2))) in
  let rec aux var =
    afficherList varlibre ; 
    if (List.mem var varlibre) then (aux (choisirVar var)) else var
  in Printf.printf "ijgvfbi";aux var
;;
